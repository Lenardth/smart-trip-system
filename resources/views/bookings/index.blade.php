...
@section('content')
    ...
    <div class="stats-strip">
        <div class="strip-card">
            ...
            <div class="strip-info">
                <h3 id="statFlights">{{ $flightCount ?? 0 }}</h3>
                <p>Flight Bookings</p>
            </div>
        </div>
        <div class="strip-card">
            ...
            <div class="strip-info">
                <h3 id="statHotels">{{ $hotelCount ?? 0 }}</h3>
        ...
        <div class="strip-card">
            ...
            <div class="strip-info">
                <h3 id="statActive">{{ $activeCount ?? 0 }}</h3>
                <p>Active Bookings</p>
            </div>
        </div>
        <div class="strip-card">
            ...
            <div class="strip-info">
                <h3 id="statSpent">${{ number_format($totalSpent ?? 0) }}</h3>
                <p>Total Spent</p>
            </div>
        </div>
    </div>
    ...
    <div class="bookings-section" id="bookingsList">
                    ...
                    <div class="booking-card status-{{ $booking->status }}"
                        ...
                        <div class="booking-inner">
                            <div class="booking-type-icon {{ $booking->type }}">
                                <i class="fas {{ $booking->type === 'flights' ? 'fa-plane' : ($booking->type === 'hotels' ? 'fa-hotel' : 'fa-route') }}"></i>
                            </div>
                            <div class="booking-info">
                                <h3>{{ $booking->title }}</h3>
                                <div class="booking-meta">
                                    <span><i class="fas fa-calendar-alt"></i> {{ $booking->departure_date?->format('M j, Y') }}</span>
                                    @if($booking->return_date)
                                        <span><i class="fas fa-calendar-check"></i> {{ $booking->return_date->format('M j, Y') }}</span>
                                    @endif
                                    <span><i class="fas fa-users"></i> {{ $booking->passengers }} {{ Str::plural('Passenger', $booking->passengers) }}</span>
                                </div>
                                <span class="booking-ref">REF: {{ $booking->reference }}</span>
                            </div>
                            <div class="booking-price">
                                <div class="amount">${{ number_format($booking->total_price, 2) }}</div>
                                <div class="per">{{ $booking->price_label ?? 'total' }}</div>
                            </div>
                        ...
                        <div class="booking-detail-row" id="detail-{{ $booking->id }}">
                            <div class="detail-grid">
                                ...
                                <div class="detail-item"><label>Reference</label><span>{{ $booking->reference }}</span></div>
                                ...
                                <div class="detail-item"><label>Payment Status</label><span>{{ $booking->payment_status ?? 'N/A' }}</span></div>
                            </div>
                        </div>
                    </div>
                @endforeach
        ...
        @else
            ...
            {{-- Demo cards shown when no real bookings exist --}}
            <div class="bookings-grid" id="bookingsGrid">
